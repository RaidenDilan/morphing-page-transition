...
const Main = () => {
  ...
  const navigate = () => {
    ...
    anime({
      ...
      d: pathRef.current?.getAttribute('pathdata:id'),
    });
  ...
  useEffect(() => {
    ...
    enterRef.current?.addEventListener('mouseenter', enterHoverInFn);
    enterRef.current?.addEventListener('mouseleave', enterHoverOutFn);

    return () => {
      enterRef.current?.removeEventListener('click', navigate);
      enterRef.current?.removeEventListener('touchenter', navigate);
      enterRef.current?.removeEventListener('mouseenter', enterHoverInFn);
      enterRef.current?.removeEventListener('mouseleave', enterHoverOutFn);
    };
  }, []);
